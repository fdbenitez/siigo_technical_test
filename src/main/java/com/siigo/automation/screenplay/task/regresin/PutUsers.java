...
public class PutUsers implements Task {
    ...
    public PutUsers(String nameService, int userId) {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        body.searchBody();
        actor.attemptsTo(
                Put.to(nameService+userId).with(request -> request.contentType(CONTENT_TYPE).body(body.generateBody()))
        );
    }

    public static PutUsers request(String nameService, int userId) {
        return instrumented(PutUsers.class, nameService, userId);
    }
